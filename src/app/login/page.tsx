...
function LoginForm() {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, padding: '40px 36px', width: '100%', maxWidth: 380, boxShadow: '0 25px 50px rgba(0,0,0,0.3)' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <input
          ...
          style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: error ? '1.5px solid #EF4444' : '1.5px solid #E5E7EB', fontSize: 14, color: '#111827', outline: 'none', boxSizing: 'border-box' as const }}
        />
      ...
      <button onClick={handleLogin} disabled={loading || !password} style={{
        ...
        color: loading || !password ? '#9CA3AF' : '#fff', fontSize: 14, fontWeight: 600,
        cursor: loading || !password ? 'not-allowed' : 'pointer',
      }}>
        {loading ? 'Signing in...' : 'Access dashboard →'}
      </button>
      <p style={{ fontSize: 11, color: '#9CA3AF', textAlign: 'center', marginTop: 20 }}>
        Internal tool for Shiprocket performance marketing team
      </p>
    </div>
  )
}
...
export default function LoginPage() {
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0F172A 0%, #1E293B 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, -apple-system, sans-serif' }}>
      <Suspense fallback={<div style={{ color: '#fff', fontSize: 14 }}>Loading...</div>}>
        <LoginForm />
      </Suspense>
    </div>
  )
